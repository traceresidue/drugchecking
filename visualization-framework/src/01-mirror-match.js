/* Nº 01 · Spectral Mirror Match — head-to-tail comparison of a submitted sample's
...
const stage=scaffold({
  ...
  dek:'Is this really what the library says it is? The submitted sample (top) is mirrored against a reference spectrum (bottom). Matched fragment ions light up; the cosine score rates the fit.',
...
stage.innerHTML=`
<div class="controls">
  <label class="muted" style="font-size:13px">Submitted sample reads as</label>
...
[sampSel,refSel].forEach(s=>{ s.className='dcf-ctl-select'; });
...
function draw(){
  ...
  if(!window.DCFDesign||DCFDesign.showTier('axis'))
    x.ticks(8).forEach(t=>{
      svg.append('text').attr('class','dcf-lbl').attr('data-tier','axis').attr('x',x(t)).attr('y',H-12).attr('fill',TOKENS.faint).attr('font-size',11).attr('text-anchor','middle').attr('font-family','ui-monospace').text(t);
    });
  svg.append('text').attr('class','dcf-lbl').attr('data-tier','legend').attr('x',W-m.r).attr('y',midY-6).attr('text-anchor','end').attr('fill',TOKENS.muted).attr('font-size',11).text('m/z →');
  svg.append('text').attr('class','dcf-lbl').attr('data-tier','legend').attr('x',m.l).attr('y',m.t-8).attr('fill',TOKENS.ink).attr('font-size',12).attr('font-weight',600).text('▲ Submitted sample');
  svg.append('text').attr('class','dcf-lbl').attr('data-tier','legend').attr('x',m.l).attr('y',H-m.b+22).attr('fill',TOKENS.muted).attr('font-size',12).attr('font-weight',600).text('▼ Library reference');

  const colS=subColor(sampSel.value), colR=subColor(refSel.value);
  function sticks(data,y,col,up){
    ...
    // label top peaks
    const labelN=window.DCFDesign?DCFDesign.peakLabelCount(5):5;
    const mzThr=window.DCFDesign?DCFDesign.mzLabelThreshold():0;
    const labels=[...data].sort((a,b)=>b.i-a.i).filter(d=>!mzThr||d.i>=mzThr).slice(0,labelN);
    if(!window.DCFDesign||DCFDesign.showTier('peak'))
      g.selectAll('text').data(labels).join('text')
        .attr('class','dcf-lbl').attr('data-tier','peak')
  ...
  if(!window.DCFDesign||DCFDesign.showTier('inline'))
    [[MS[sampSel.value].mw,yUp,colS,true],[MS[refSel.value].mw,yDn,colR,false]].forEach(([mw,y,c,up])=>{
      svg.append('text').attr('class','dcf-lbl').attr('data-tier','inline').attr('x',x(mw)).attr('y',up?m.t+12:H-m.b-2).attr('text-anchor','middle').attr('font-size',10).attr('fill',c).text('M⁺• '+mw);
    });

  const score=cosine(top,bot);
  const verdict=score>0.9?['Strong match',sigColor('ok')]:score>0.6?['Partial match — check further',sigColor('watch')]:['Poor match',sigColor('alert')];
