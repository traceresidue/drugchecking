...
Plotly.newPlot('plot',[{
  ...
  colorscale:[[0,'#0b0e14'],[0.1,'#2a1a3a'],[0.3,'#7a3a5a'],[0.6,'#ff5c7a'],[0.85,'#ff8a5c'],[1,'#ffd166']],
  showscale:false,contours:{z:{show:true,usecolormap:true,width:1,project:{z:false}}}
}],{
  paper_bgcolor:'rgba(0,0,0,0)',
  scene:{
    xaxis:{title:'month →',color:'#8b94a8',gridcolor:'#26304a',backgroundcolor:'#0b0e14',showbackground:true,
      tickvals:months.map((m,i)=>i).filter((_,i)=>i%4===0),ticktext:months.filter((_,i)=>i%4===0).map(m=>{const[y,mo]=m.split('-');return `${['','Jan','Feb','Mar','Apr','May','Jun','Jul','Aug','Sep','Oct','Nov','Dec'][+mo]} '${y.slice(2)}`;})},
