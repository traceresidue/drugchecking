...
function draw(){
  ...
  svg.append('text').attr('x',W/2).attr('y',H-2).attr('text-anchor','middle').attr('fill',TOKENS.muted).attr('font-size',11).text('wavenumber (cm⁻¹) — high to low');
  ...
  g.append('stop').attr('offset','0%').attr('stop-color',TOKENS.fent).attr('stop-opacity',.4);
  g.append('stop').attr('offset','100%').attr('stop-color',TOKENS.fent).attr('stop-opacity',.03);
  svg.append('path').datum(resid.y).attr('d',area).attr('fill','url(#rg)');
  svg.append('path').datum(resid.y).attr('d',d3.line().x((d,i)=>x(resid.x[i])).y(d=>y(d)).curve(d3.curveBasis)).attr('fill','none').attr('stroke',TOKENS.fent).attr('stroke-width',1.8);
  ...
  document.getElementById('leg').innerHTML=
    `<span><i style="background:${TOKENS.muted}"></i>full mixture</span>
     <span><i style="background:${TOKENS.fent}"></i>residual after subtraction (${[...mix].filter(r=>!subtracted.has(r)).length} components left)</span>
